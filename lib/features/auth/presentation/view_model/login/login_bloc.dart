...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  void _onLoginSubmitted(LoginSubmittedEvent event, Emitter<LoginState> emit) async {
    ...
    result.fold(
      ...
      (response) {
        emit(state.copyWith(isLoading: false, isSuccess: true, isLoggedIn: true));
        shakeCubit.startListening(); // ✅ Start shake detection globally
        Navigator.pushReplacement(
          event.context,
          MaterialPageRoute(
            builder: (context) =>
                BlocProvider.value(value: movieBloc, child: const HomeView()),
          ),
        );
      },
    );
  ...
  String? _validateLoginForm({required String usernameOrEmail, required String password}) {
